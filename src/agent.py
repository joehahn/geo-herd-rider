...
def _scout_once(client, anchor, chunk: list[dict], rblock: str, label: str) -> list[dict]:
    ...
    user = (f"Week ending {anchor.date()}. Headlines:\n\n{_block(chunk)}\n{rblock}\n"
            ...
            "NOT happened yet. Which thing depends on the KIND of driver:\n"
            "    ACUTE (a single datable event that cleanly resolves -- a bill signed, a merger "
            "closed, a ceasefire, a chokepoint reopened): name the event whose happening would END "
            "this thesis -- a decision date, a vote, a ruling, a deal still to close.\n"
            "    STRUCTURAL (a multi-year regime shift or adoption curve that keeps throwing off NEW "
            "milestones -- a standing export-curb regime, a build-out, a technology getting cheaper "
            "and more common year after year): name the NEXT SCHEDULED MILESTONE the driver throws "
            "off, NOT the far-off finish line. A driver whose completion is years away still "
            "qualifies if a concrete next step is ahead (a funding tranche, a capacity milestone, a "
            "first commercial contract, a scheduled ruling). Do NOT drop it merely because the thesis "
            "does not end when that milestone lands.\n"
            "DROP the candidate only if you can name NEITHER -- i.e. the only thing you can point to "
            "is an event that ALREADY happened (the contract WAS awarded, the merger WAS announced, "
            "earnings WERE reported, the approval WAS granted) and nothing concrete is scheduled "
            "ahead: write \"none\" and drop it. A catalyst in the past tense with nothing behind it "
            "is news the market has already priced. Propose every survivor; there is no quota.\n"
