...
def _stateful_watch(scans: dict, exit_after_quiet_weeks: int = 0) -> dict:
    ...
    for a in anchors:
        ...
        for t in live:                       # (re)enter / refresh
            ...
            if exit_after_quiet_weeks:       # priced-in: count consecutive no-new-development weeks
                quiet[t] = 0 if newdev.get(t, True) else quiet.get(t, 0) + 1
...
def backtest(scans: dict, fm: dict, capital: float = 50_000.0, daily: bool = False,
    ...
    anchors = list(scans)
    watch = _stateful_watch(scans, int(fm.get("exit_after_quiet_weeks", 0) or 0))  # sticky hold + priced-in exit
