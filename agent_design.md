...
One catalyst usually has several credible vehicles (a European-rearmament shock lifts Rheinmetall + BAE / Saab / Thales; a rare-earth curb lifts several miners). The scout names the purest vehicle as the primary `ticker` and lists its direct same-catalyst peers in a `peers` field; the event-agent proposes the whole basket and the mechanical optimizer sizes them, dropping the weak ones (the LLM never forecasts *which* peer wins). A peer must share the **same catalyst**, so a basket cannot drift into an unrelated gem (a naive "just propose more names" version drifted across catalysts and lost ~45% of return). **That was an aspiration, not an enforced invariant, until 2026-08-30** — the word "structurally" stood here and was wrong. Measured over the last 12 scans of `cbt_3yr_v24_wirelede`, **8 of 40 vehicle additions (20%) named a ticker appearing in no headline among that event's matched articles**: `XOM` was added to "the Iran war disrupts oil supplies" with Exxon in 0 of 22 matched articles, and `WSM` to "RH pivots supply chain amid tariffs" with Williams-Sonoma in 0 of 319. Those are "oil disruption so Exxon benefits" and "RH has trouble so its competitor matters" — the retired causal ladder, leaking back in through the vehicle list. It is enforced now: `agent._named_in` admits an ADDED vehicle only if its symbol or company name appears in a TITLE of the event's uncapped relevance match (`_filter_event(..., cap=0)` — `cap` is a reading budget, not a relevance judgement). Existing vehicles pass untouched, so an agent can still drop one. **It is a correctness fix and is expected to be P&L-neutral or slightly negative**: never-named tickers do not underperform (median $2,655 vs $2,362 on v24; $2,975 vs $2,235 on v23). Its strength is bounded by how wide that match is — see TODO.md on `_filter_event` over-collection. A/B honesty: on RNMBY the basket formed cleanly (RNMBY + BAE + Saab + Thales) yet the *single* purest name still won (+251% vs +235%) — baskets help when the best vehicle is ambiguous, not when it is already the clear winner.
