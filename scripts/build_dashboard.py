...
ACTIVE_GEMS = {"MP", "BWET", "GEO", "MSTR", "RNMBY", "GDX", "SMR"}   # the event-driven gems we're actively tuning on — colored + first-row; rest greyed
...
Promise.resolve({{DATA}}).then(D=>{
  ...
  if(capEl){
    if(!nap){ capEl.innerHTML="No agents to score this window."; }
    else{
      const APc=AP.slice().sort((a,b)=>b.ret-a.ret), pct=v=>(v>=0?"+":"")+Math.round(v*100)+"%";
      const list=APc.map(r=>`<b style="color:${r.ret>=0?'#2ca02c':'#d62728'}">${r.ticker} ${pct(r.ret)}</b>`).join(", ");
      const gem=(D.overlay_ticker||"").toUpperCase(), gemR=APc.find(r=>r.ticker===gem);
      const best=APc[0], worst=APc[APc.length-1], bits=[];
      bits.push(gemR?`the gem ${gem} was a ${gemR.ret>0?"winning":"losing"} call (${pct(gemR.ret)})`
...
Promise.resolve({{DATA}}).then(D=>{
  ...
  if(BO && BO.models && BO.models.length){
    ...
    const traces=[{type:"scatter", mode:"lines+markers+text", name:"(Sum 6 gems)/4", x:labels,
      y:idx.map(i=>BO.sum_curated[i]/4), line:{color:"#d62728",width:2.8}, marker:{size:9},
      text:idx.map(i=>gems.map(g=>g+(BO.caught[BO.models[i]][g]?"✓":"✗")).join(" ")),
      textposition:"top center", textfont:{size:9},
      hovertemplate:"%{x}<br>(Sum)/4 $%{y:,.0f}<extra></extra>"}];
