...
def _gem_universe() -> list:
    ...
    for g in spec["gems"]:
        ...
        out.append({"ticker": tk, "name": g.get("name", ""), "start": trig, "scanned": tk in scanned,
                    "mult": round(float(s.max()) / base, 2),
                    "x": [d.strftime("%Y-%m-%d") for d in s.index],
                    "y": [round(float(v) / base * 100, 1) for v in s]})
...
<body><div class="wrap">
 ...
 <h2>Plot 1 — Each gem's price over its window <span class="sub" style="font-size:13px;font-weight:400">— ALL candidate gems, indexed to 100 at each gem's trigger (24-mo window). Scanned gems are bold + labelled with their peak multiple; the dashed line is the 3× cherry-pick bar. Use this to focus on the big movers (SMR/MP/BWET 3×+) and stop tuning on weak ones (GDX ~2×).</span></h2>
...
<script>
 const S={{GEMSPLOT}}, pal=["#2980b9","#c0392b","#27ae60","#8e44ad"];
 if(S&&S.length){
  let ci=0;
  const traces=S.map(s=>({x:s.x,y:s.y,name:s.ticker+" "+s.mult+"×",mode:"lines",
    line:{color:s.scanned?pal[(ci++)%pal.length]:"#c3c3c3",width:s.scanned?2.6:1.2,dash:s.scanned?"solid":"dot"},
    opacity:s.scanned?1:0.6,connectgaps:false,
    hovertemplate:s.ticker+" %{y:.0f} ("+s.mult+"× peak)<br>%{x|%Y-%m-%d}<extra></extra>"}));
  Plotly.newPlot("gemsplot", traces,
   {margin:{l:46,r:14,t:6,b:34},legend:{orientation:"h",y:1.16,font:{size:11}},
    xaxis:{type:"date"},yaxis:{title:"indexed (trigger=100)",zeroline:false},hovermode:"closest",
    shapes:[{type:"line",xref:"paper",x0:0,x1:1,y0:300,y1:300,line:{color:"#888",width:1,dash:"dash"}}],
    annotations:[{xref:"paper",x:0.01,y:300,yanchor:"bottom",text:"3× cherry-pick bar",showarrow:false,font:{size:10,color:"#888"}}]},
   {displayModeBar:false,responsive:true});
 }
</script>
</div></body></html>
