...
def build_landing() -> None:
    """Landing page at docs/index.html: an overview plot of each gem's price over its window +
    one card per built gem (scans docs/<gem>/data.json), ordered chronologically."""
    rows, series = [], []
    for sub in sorted(OUT_DIR.glob("*/data.json")):
        ...
        # overview plot: the gem TICKER's price over its window, indexed to 100 at the window start
        ov, dts = d.get("overlay") or [], d.get("dates") or []
        base = next((v for v in ov if v), None)
        if base and dts:
            series.append({"ticker": d.get("overlay_ticker") or d.get("gem"), "start": dts[0],
                           "x": dts, "y": [round(v / base * 100, 1) if v else None for v in ov]})
    rows.sort(key=lambda r: r["window"])          # chronological (earliest window first)
...
<body><div class="wrap">
 ...
 <h2>Each gem's price over its window <span class="sub" style="font-size:13px;font-weight:400">— indexed to 100 at window start, on a shared timeline (what each gem did, and when)</span></h2>
 <div id="gemsplot" style="height:340px;margin-bottom:6px"></div>
 <div class="grid">{{CARDS}}</div>
 <p class="foot">geo-herd-rider · generated by <code>scripts/build_dashboard.py --all</code></p>
<script>
 const S={{GEMSPLOT}}, pal=["#2980b9","#27ae60","#c0392b","#8e44ad","#e67e22","#16a085"];
 if(S&&S.length) Plotly.newPlot("gemsplot", S.map((s,i)=>({x:s.x,y:s.y,name:s.ticker,mode:"lines",
   line:{color:pal[i%pal.length],width:2},connectgaps:false,
   hovertemplate:s.ticker+" %{y:.0f}<br>%{x|%Y-%m-%d}<extra></extra>"})),
   {margin:{l:46,r:14,t:6,b:34},legend:{orientation:"h",y:1.14},
    xaxis:{type:"date"},yaxis:{title:"indexed (start=100)",zeroline:false},hovermode:"closest"},
   {displayModeBar:false,responsive:true});
</script>
</div></body></html>
"""
