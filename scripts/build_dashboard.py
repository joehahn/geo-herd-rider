...
<body><div class="wrap">
 ...
 <h2>Plot 1 — Each gem's price over its window <span class="sub" style="font-size:13px;font-weight:400">— ALL candidate gems, normalized to 1.0× at each gem's trigger (24-mo window); the y-axis is the price multiple. Scanned gems are bold + labelled with their peak multiple; the dashed line is the 3× cherry-pick bar. Use this to focus on the big movers (SMR/MP/BWET 3×+) and stop tuning on weak ones (GDX ~2×).</span></h2>
 ...
 if(S&&S.length){
  ...
  Plotly.newPlot("gemsplot", traces,
   {margin:{l:46,r:14,t:6,b:34},legend:{orientation:"h",y:1.16,font:{size:11}},
    xaxis:{type:"date"},yaxis:{title:"multiple (start = 1.0×)",zeroline:false},hovermode:"closest",
    shapes:[{type:"line",xref:"paper",x0:0,x1:1,y0:3,y1:3,line:{color:"#888",width:1,dash:"dash"}}],
    annotations:[{xref:"paper",x:0.01,y:3,yanchor:"bottom",text:"3× cherry-pick bar",showarrow:false,font:{size:10,color:"#888"}}]},
   {displayModeBar:false,responsive:true});
 }
</script>
</div></body></html>
