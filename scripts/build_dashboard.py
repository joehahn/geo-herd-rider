...
<body><div class="wrap">
 ...
 <h2>What it cost to build this</h2>
 <p class="sub">Every LLM call in the pipeline — the trigger scout and the causal-ladder
   mapper — is metered. Web search is billed separately (it dominates on cheap models).</p>
 <div id="costs"></div>
...
fetch("data.json").then(r=>r.json()).then(D=>{
  ...
  document.getElementById("costs").innerHTML =
    `<div class="card" style="max-width:360px"><div class="k">total LLM spend · ${c.n_calls||0} calls</div>`
    + `<div class="v">${usd(c.total_usd)}</div>`
