...
def build_landing() -> None:
    ...
    rows.sort(key=lambda r: r["window"])           # cards: chronological (earliest window first)
...
<script>
 const S={{GEMSPLOT}}, pal=["#2980b9","#c0392b","#27ae60","#8e44ad","#e67e22","#16a085",
   "#e6b800","#2c3e50","#e84393","#00b894","#0984e3","#6c5ce7","#d35400"];
 if(S&&S.length){
  const y3=Math.log10(3);   // log axis: shape/annotation y in data coords = log10(value)
  const traces=S.map((s,i)=>({x:s.x,y:s.y,name:s.ticker+" "+s.mult+"×",mode:"lines",
    line:{color:pal[i%pal.length],width:s.scanned?2.8:1.6},connectgaps:false,
  ...
  Plotly.newPlot("gemsplot", traces,
   {margin:{l:46,r:14,t:6,b:34},legend:{orientation:"h",y:1.16,font:{size:11}},
    xaxis:{type:"date"},yaxis:{title:"multiple (start = 1.0×, log)",type:"log",zeroline:false},hovermode:"closest",
