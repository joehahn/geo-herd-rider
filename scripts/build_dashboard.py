...
ACTIVE_GEMS = {"SMR", "MP", "BWET"}   # the gems we're actively tuning on — colored + first-row; rest greyed
...
def _gem_universe() -> list:
    ...
    return sorted(out, key=lambda r: -r["mult"])   # biggest movers first
...
def build_landing() -> None:
    ...
    def card(r):
        ...
        return (f'<a class="gcard{mut}" href="{r["url"]}"><div class="gt">{r["gem"]}</div>'
...
<body><div class="wrap">
 ...
 <h2>Plot 1 — Each gem's price over its window <span class="sub" style="font-size:13px;font-weight:400">— ALL candidate gems, normalized to 1.0× at each gem's trigger (24-mo window); the y-axis is the price multiple (log scale). Scanned gems are bold + labelled with their peak multiple; the dashed line is the 3× cherry-pick bar. Use this to focus on the big movers (SMR/MP/BWET 3×+) and stop tuning on weak ones (GDX ~2×).</span></h2>
...
<script>
 const S={{GEMSPLOT}}, pal=["#2980b9","#c0392b","#27ae60","#8e44ad","#e67e22","#16a085"];
 if(S&&S.length){
  const y3=Math.log10(3);   // log axis: shape/annotation y in data coords = log10(value)
  let ci=0;
  const traces=S.map(s=>({x:s.x,y:s.y,name:s.ticker+" "+s.mult+"×",mode:"lines",
    line:{color:s.active?pal[(ci++)%pal.length]:"#d2d2d2",width:s.active?2.8:1},
  ...
  const xs=S.filter(s=>s.active).flatMap(s=>[s.x[0],s.x[s.x.length-1]]).sort();   // x-range = active gems' span
  ...
  Plotly.newPlot("gemsplot", traces,
   {margin:{l:46,r:14,t:6,b:34},legend:{orientation:"h",y:1.16,font:{size:11}},
    xaxis:{type:"date",range:xr},yaxis:{title:"multiple (start = 1.0×, log)",type:"log",zeroline:false},hovermode:"closest",
    shapes:[{type:"line",xref:"paper",x0:0,x1:1,y0:y3,y1:y3,line:{color:"#888",width:1,dash:"dash"}}],
    annotations:[{xref:"paper",x:0.01,y:y3,yanchor:"bottom",text:"3× cherry-pick bar",showarrow:false,font:{size:10,color:"#888"}}]},
   {displayModeBar:false,responsive:true});
 }
</script>
</div></body></html>
