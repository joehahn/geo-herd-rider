...
<header>
  ...
  <p class="sub">Eight LLMs made the same {n_arm} decisions over 100,000 articles, graded blind by a
     frontier model. The most expensive one finished last.</p>
</header>
...
<section>
<h2>1. Quality peaks in the middle</h2>
...
<section>
<h2>2. Cost says nothing about speed</h2>
...
the eight.</p>
<p>The second surprise is a practical one. <b>Price and speed are unrelated.</b> The cheapest model was
...
<p>Section 1's scores are a synthesis of the three tests detailed here, higher is better.</p>
...
#c1 {{ height: 480px; }}   /* horizontal, 8 categories -- needs more vertical room */
p {{ margin: 0 0 13px; }}
...
function draw() {{
  ...
  Plotly.react('c2', [{{type:'bar', x:ix, y:BO.map(r=>r.minutes), marker:{{color:'#22d3ee'}},
  ...
  Plotly.react('c1', [{{type:'bar', orientation:'h',
