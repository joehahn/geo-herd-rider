...
- An **event** is the real-world thing that is happening; the **storyline** this solution tracks (e.g. "Hormuz blockade").
- A **catalyst** is the storyline's **spine**: the single continuous driver running through the whole event, which *resolves once* (that resolution is the **exit**). For "Hormuz blockade" the spine is *Iran's push to close the Strait of Hormuz*, and it resolves at the **ceasefire**. Several tickers can **share** one catalyst (a *same-catalyst* basket).
