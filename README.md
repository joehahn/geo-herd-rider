...
The pipeline is built end-to-end and runs over three years of historical news. The [dashboards](#dashboards) carry the current figures, which move whenever the code does; below is how to read them.

**Results so far.**

- *Backtest (CBT), three years of archived news replayed month by month.* The curated book finishes well ahead of both controls, an equal-dollar buy-and-hold of the starter basket and SPY. This is the headline result and also the most contaminated one: the curator model was trained past these events, and a backtest steered by returns on known history is how you overfit.
- *Bootstrap (CBS), the transition corpus.* The same machinery on the least contaminated data available is **losing**, behind both controls. The loss traces to the names this curator introduced itself rather than to the tickers it inherited from the backtest, so the curation is what costs money. A handful of curations is far too few to conclude anything in either direction, and it is reported here because it is the honest result, not the flattering one.
...
- **Every historical number here is a ceiling, and cannot be made otherwise.** Reading old news is not the same problem as reading this week's. No search tool offers true point-in-time retrieval: date filters leak articles published after the cutoff, results are ranked by what later became famous, and what comes back is today's edited page rather than what was published at the time. A date-honest index avoids the leak but under-indexes the niche trade press, so it finds a gem only once the mainstream has piled in. On top of that, the curator model was trained past these events. A clean *retrospective* test is therefore not achievable at all, every backtest figure is reported as the ceiling it is, and the forward paper trade is the verdict.
