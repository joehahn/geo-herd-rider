...
- A **gem** is an in-demand ticker that benefits from the event. The **scout** names the gems (the purest name plus its same-catalyst peers), and the **matcher** assigns them to the event's **basket**, which can evolve as the event unfolds (the event is pinned to the catalyst, not to a ticker). The **event-agent** then proposes the basket, and the optimizer sizes it and drops the weak tickers.
