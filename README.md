...
These beats are deliberately gem-agnostic, not reverse-engineered from the gems known to have won (which would inflate recall and mean nothing forward). The full retrieval decision matrix is in [`agent_design.md`](agent_design.md#retrieval-gdelt-and-seeds-current).
