...
- **Backtest — replaying history to score this solution.** Here a normal web search is *poison*: searching old news *today* silently re-imports the future — its date filters leak post-cutoff articles, its results are ranked by what *later* became famous, and it returns today's edited page. **Which is why this solution uses GDELT + Wayback:** **GDELT** is the only date-honest discovery index — server-enforced date bounds, and results ordered *by date, not relevance* (so a gem's early article isn't boosted because it later mooned). We query it with a **fixed, gem-agnostic list of ~15 beats** — market-wide superlatives and standard sector themes, **never a ticker symbol** (the full list is in [`agent_design.md`](agent_design.md#retrieval-gdelt-and-seeds-current)). GDELT's catch is that it returns **headlines only**, and a headline names the *theme*, rarely the *ticker* (the "(BWET)" lives in the lede). **Wayback** recovers the ticker look-ahead-clean — it fetches each GDELT hit's *as-of-date* archived lede (URL-keyed archival, so none of web search's three leaks; opt-in `--enrich`, under validation):
  ```
  Wayback CDX:  latest snapshot of <url> with to=20260213  ->  fetch lede ("...Breakwave Tanker Shipping ETF (BWET)...")
  ```
  **Seeds** cover the *other* gap — the niche early pieces GDELT never indexes at all (Wayback can only enrich URLs GDELT already returned); they're injected at their true dates and remain the default until `--enrich` is validated.
