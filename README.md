...
- **Event** — the real-world thing that is happening; the **storyline** this solution tracks (e.g. "Hormuz blockade").
- **Catalyst** — the storyline's **spine**: the single continuous driver running through the whole event, which *resolves once* (that resolution is the **exit**). For "Hormuz blockade" the spine is *Iran's push to close the Strait of Hormuz*, and it resolves at the **ceasefire**. Several tickers can **share** one catalyst (a *same-catalyst* basket).
- **Milestones** — the **vertebrae** on that spine: the developments along the way (*protests in Iran → a US carrier group to the Med → strikes on Iran → the Hormuz closure itself*) that keep the catalyst *live* and bump the event's `conviction` — beats of the one event, not separate catalysts.
