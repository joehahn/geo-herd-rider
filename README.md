...
When **backtesting**, each weekly news-grab is fetched from **GDELT**, whose date bounds are server-enforced — so a scan as-of a past week sees only news published by then and can't leak future news into the backtest. However the news returned by GDELT tends to miss the niche early under-the-radar pieces, so those are **seeded** back at their true dates. When this solution is run **forward in time** (week to week, for real), that look-ahead problem disappears — "search now for a just-happened gem" is clean by construction — so the firehose there is **Anthropic web search** instead. To keep the two comparable, the backtest's GDELT queries are **gem-agnostic** — superlatives ("best performing stock") plus the standard market beats the forward search prompt already names — *not* terms reverse-engineered from the gems we know won (which would inflate recall and mean nothing forward). See [`agent_design.md`](agent_design.md#retrieval-gdelt-and-seeds-current) for the retrieval mechanics.
