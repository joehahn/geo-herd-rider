...
- An **event** is the real-world thing that is unfolding, and it is the storyline that this solution tracks (e.g. "Hormuz blockade").
- A **catalyst** is the event's spine, it is the continuous driver that runs through the entire event, preferably one that will ultimately resolve, with that resolution known as the **exit**. For the "Hormuz blockade" event the spine is *Iran's push to close the Strait of Hormuz*, and that event resolves with a *ceasefire*. An event can have multiple tickers associated with it, and they all share the same catalyst.
- **Milestones** are the **vertebrae** on that spine: the developments along the way (*protests in Iran → a US carrier group to the Med → strikes on Iran → the Hormuz closure itself*) that keep the catalyst *live* and feed the event's conviction (next). They're beats of the one event, not separate catalysts.
- **Conviction** (1–10) is the event-agent's weekly rating of how *live and still-under-owned* the catalyst is: a fresh milestone lifts it, silence or a priced-in market decays it. It's the only number the LLM emits (an ordinal standing, never a size or return), and it's what the competitive cull ranks on ([details](#conviction-how-its-scored-and-how-it-decays)).
