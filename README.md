...
The assembly line runs once per **rebalance period** and marches period by period across the era. Each pass re-reads the firehose, each event-agent re-asks whether its event's thesis is still live, names the vehicle or vehicles that best express it, and the optimizer rebalances. Those vehicles can change as the event unfolds. **Sizing is mechanical**: the AI names tickers and makes the hold-or-exit call, and never sets a position size.
