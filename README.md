...
- **Event** — the real-world thing that happened or is happening; the **storyline** this solution tracks (e.g. "Hormuz blockade").
- **Catalyst** — the storyline's **spine**: the single driving through-line (a war, a China rare-earth curb, a bill, a supply shock) that *resolves once* — and that resolution is the **exit**. Several tickers can **share** one catalyst (a *same-catalyst* basket).
- **Milestones** — the **vertebrae** on that spine: the developments along the way (an escalation, a signed step, a fresh headline) that keep the catalyst *live* and bump the event's `conviction` — beats of the one event, not separate catalysts.
