...
- Note that there are two kinds of **agent**: (i) the **event-agents** described that have fluid baskets; and (ii) the **always-on floor agents**, namely an ever-present **SPY floor agent** as well as the **configurable defensive agent** that favors GLD by default. These always-on agents have a **fixed conviction** (default 5), which gives the optimizer the opportunity to fund safe harbors like SPY and/or GLD during moments when the event-agents are all low-conviction (see [the floors](#the-signal-and-its-jobs)).
