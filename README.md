...
- An **exit** is where the spine ends. The catalyst is resolved (e.g. the war ends, the bill is signed, the chokepoint reopens). For example, a ceasefire triggers an exit from the "Hormuz blockade" event. The event-agent drops the position the instant it judges the catalyst done, even if the coverage is still loud, because a resolved catalyst can no longer drive the ticker.
