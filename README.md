...
**How far to trust them.** No search tool offers true point-in-time retrieval — date filters leak articles published after the cutoff, results are ranked by what later became famous, and what comes back is today's edited page rather than what was published at the time — so a clean retrospective test is not achievable at all, and every historical figure here is a ceiling. Nor can one run settle anything: the same settings run twice, differing only in LLM sampling, produced median finals of **$117,200 and $62,997**, so any difference under about 2× is treated as unmeasurable and changes are judged on mechanism — coverage, cancellation, cull-at-birth rate, what a code change does to the articles the scout actually reads — rather than on P&L. A hypothesis is believed only once it survives a second, independent curation, which is brutal in practice: of eight plausible explanations for why the book loses money, one survived. A measurement that changes no decision gets deleted, however sensible it sounded — each agent used to score its conviction from 1 to 10 until that number was replayed against its own null and ranked at random, because a confidence score an LLM is happy to produce is not the same thing as information. And a published page is gated so it can only ever describe one corpus, curation and profile, with `python scripts/check_canon.py` answering "is everything consistent?" in one command.
