...
- A **thesis** is the statement that connects the event to a ticker (e.g. this catalyst is causing that ticker to rise), and it is authored by the **scout**.
- A **gem** is a same-catalyst ticker that expresses the event. The **scout** names the gems (the purest name plus its same-catalyst peers), and the **matcher** assigns them to the event's **basket**, which can evolve as the event unfolds (the event is pinned to the catalyst, not to a ticker). The **event-agent** then proposes the basket, and the optimizer sizes it and drops the weak tickers.
