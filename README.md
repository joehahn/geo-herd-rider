...
Every LLM call routes through a provider-agnostic seam (`src/llm.py`), so the same pipeline runs on Anthropic (Opus/Sonnet/Haiku) or any OpenRouter model via the **`model:` knob in `investor_profile.md`** (`optimizer.resolve_curator_model` maps `mimo|sonnet4|sonnet5|opus|llama4|deepseek|grok4` → provider + id), with structured-output JSON schemas keeping cheap models' output clean. Each scan stamps a `<scan>.meta.json` sidecar so every dashboard shows *which* model produced that book. A **7-model bake-off** (top plot on the sweeps page) re-scans each model's 3 gems under the current prompts and re-scores them on shared price panels. The current read: **the top is a near-tie and strongly config-sensitive** — at the live config Llama-4 and Sonnet-5 finish within ~0.3% atop the sum, and the spy-floor / lookback / cap can reorder the leaders, so treat the bake-off as **directional**, not a verdict. The **chosen curator is Sonnet-5**, picked for its **selectivity + early-anticipation catch** — it names *few, high-conviction* events and, with the anticipation-gate + hold-through-silence prompt fixes, catches a gem while its catalyst is still forming (the RNMBY case). The cheap open-weights (Llama-4, MiMo, DeepSeek, ~$0.1–0.4 per 3-gem scan) can *tie* the frontier on one config but tend to **sprawl** on others (naming many events, funding losers), so a strong open-weight showing is partly config-luck. The live curator is the **`model:` knob** in `investor_profile.md` (currently Sonnet-5). Every call's cost is priced into `data/llm_costs.csv`.
...
**Results so far.**
- *News-derived gem set (6 gems, event-first engine — the current state):* every gem now runs on **real news-derived seeds**, not synthetic. Five are **caught on discrete catalysts** and ride cleanly to a resolution-exit — **SMR** (the ADVANCE Act → hard-exit on the signing, the textbook arc), **BWET** (Hormuz closure → ceasefire exit), **MP** (China rare-earth curbs + the DoD deal), **GEO+MSTR** (the 2024 election, both vehicles — MSTR entering *early* via the dated-anticipation gate and exiting at the vote), and **RNMBY** (European rearmament — a dial-up detection test). **GDX is the deliberate negative control**: its 3× run was a diffuse macro theme (central-bank gold buying, safe-haven) with no discrete catalyst until a late gold-specific tariff, so the gate correctly *declines it early* and catches it only at the blow-off top. All returns are **config-dependent hindsight upper bounds** (settled on the [sweeps dashboard](https://joehahn.github.io/geo-herd-rider/sweeps/)).
...
- *Model bake-off (3 gems, 7 models, current prompts; top plot on the sweeps dashboard):* the top is a **near-tie and config-sensitive** — at the live config Llama-4 and Sonnet-5 finish within ~0.3%, with Sonnet-4/DeepSeek/Opus behind and Grok-4/MiMo trailing. Chosen curator = **Sonnet-5** (`model:` knob), for selectivity + the early-anticipation catch.
