...
- **Event** — the real-world thing that happened (a war, a China rare-earth curb, a bill, a supply shock), tracked from entry to exit by one **event-agent** that rates its `conviction`. *Same thing, different hat:* when we mean its **driver** — the part that *resolves* (which triggers the exit) or that several tickers **share** (a *same-catalyst* basket) — we call it the **catalyst**. A catalyst is a single *through-line* that **resolves once** (that resolution is the exit); the developments along the way — each escalation or **milestone** — keep it *live* and bump `conviction`, and are **not** separate catalysts. *(A written description of it is the **thesis**, next.)*
