...
- An **event** is the real-world thing that is happening, it is the storyline that this solution tracks (e.g. "Hormuz blockade").
- A **catalyst** is the event's **spine**, it is the single continuous driver that runs through the entire event, preferably one that will resolve, with that resolution known as the **exit**. For the "Hormuz blockade" event the spine is *Iran's push to close the Strait of Hormuz*, and that event resolves with a **ceasefire**. Note that an event can have multiple tickers associated with it, and they would share the same catalyst.
