...
- A **catalyst** is the event's **spine**, it is the single continuous driver that runs through the entire event, preferably one that will ultimately resolve, with that resolution known as the **exit**. For the "Hormuz blockade" event the spine is *Iran's push to close the Strait of Hormuz*, and that event resolves with a **ceasefire**. Note that an event can have multiple tickers associated with it, and they would share the same catalyst.
- **Milestones** are the **vertebrae** on that spine: the developments along the way (*protests in Iran → a US carrier group to the Med → strikes on Iran → the Hormuz closure itself*) that keep the catalyst *live* and bump the event's `conviction` — beats of the one event, not separate catalysts.
