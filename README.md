...
- *Model bake-off (6 gems, 7 models, current prompts; top plot on the sweeps dashboard):* by summed value DeepSeek tops it ($947k) but **only via sprawl** on the two dirtiest gems (RNMBY dial-up, GDX negative control); **Sonnet-5 wins the four cleanly-caught gems** and catches all six. The sum rewards sprawl under a permissive `max_agents`, so precision beats gross value as the test. Chosen curator = **Sonnet-5** (`model:` knob), for selectivity + the early-anticipation catch. Cost note: the 3 Anthropic models were ~$170 of the ~$180 run (Sonnet-5 $75, Opus $66); the open-weights cost pennies.
