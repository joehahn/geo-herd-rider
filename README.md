...
- Note that there are two kinds of **agent**: (i) the **event-agents** described above, which have fluid baskets; and (ii) the **always-on floor agents**: an ever-present **SPY floor agent** and a **configurable defensive agent** (which favors GLD by default). The always-on agents have **fixed conviction** (default 5), which gives the optimizer the opportunity to fund safe harbors like SPY and/or GLD in moments when the event-agents are all low-conviction (see [the floors](#the-signal-and-its-jobs)).
