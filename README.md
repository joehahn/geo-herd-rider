...
- An **event** is the real-world thing that is unfolding, and it is the storyline that this solution tracks (e.g. "Hormuz blockade"). The **scout** flags it, and the **matcher** gathers all its same-catalyst tickers into that one event.
- A **catalyst** is the event's spine, written by the **scout**; it is the continuous driver that runs through the entire event, preferably one that will ultimately resolve, with that resolution known as the **exit**. For the "Hormuz blockade" event the spine is *Iran's push to close the Strait of Hormuz*, and that event resolves with a *ceasefire*. An event can have multiple tickers associated with it, and they all share the same catalyst.
- **Milestones** are the vertebrae on the spine, they are the developments along the way (*protests in Iran → a US carrier group to the Med → strikes on Iran → the Hormuz closure itself*) that keeps the catalyst *live* and feeds the event's **conviction**; the **event-agent** tracks them week to week.
- **Conviction** is the event-agent's weekly rating (1–10) of how live and still-under-owned the catalyst is. Conviction is driven by the news: a fresh milestone can lift it, while silence, or news that the market has priced the move in, can cause it to decay over time.
- An **exit** is where the spine ends: the catalyst *resolves* (the war ends, the bill is signed, the chokepoint reopens; for "Hormuz blockade," the ceasefire). The event-agent drops the position the instant it judges the catalyst done, even if the coverage is still loud, because a resolved catalyst can no longer drive the ticker.
- A **thesis** is the written claim tying the event to a ticker (*this* catalyst makes *this* ticker rise); authored by the **scout**.
- A **vehicle / gem** is a same-catalyst ticker that expresses the event *right now* (the purest name or one of its peers); together they form the **basket**, which can evolve as the event unfolds (the event is pinned to the catalyst, not to a ticker). The **matcher** assigns each gem to its event, the **event-agent** proposes the resulting basket, and the optimizer sizes it and drops the weak vehicles.
