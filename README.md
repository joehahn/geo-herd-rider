...
In these backtests the firehose is realized by date-honest **GDELT** headlines (its date bounds are server-enforced, so a scan sees only what was published by then), plus the early under-the-radar pieces GDELT misses, **seeded** back at their true dates — see [`agent_design.md`](agent_design.md#retrieval-gdelt-and-seeds-current) for the retrieval mechanics.
