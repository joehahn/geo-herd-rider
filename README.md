...
flowchart TD
    ...
    E["🎯 Watchlist<br/>gathers the surviving events' gems (top-N after the max_agents cull) for possible funding"]
...
- [**Parameter-sweep dashboard**](https://joehahn.github.io/geo-herd-rider/sweeps/) — how the outcome varies as each of the **8 solution parameters in `investor_profile.backtest.md`** (the dev/backtest config; its frozen live twin is `investor_profile.forward.md`) is swept one at a time (vs the Sum-SPY benchmark): the curator **`model`** (a 7-model LLM bake-off), plus `lookback_period_days`, `concentration_cap`, `min_trade_size`, `risk_aversion`, `max_agents`, `spy_agent_conviction`, and `defensive_agent_conviction`.
