...
- A **gem** is an in-demand ticker that benefits from the event. The **scout** names the gem or gems, and the **matcher** assigns the same-event gems to the event's **basket** which can evolve as the event unfolds since the event is pinned to the catalyst and not to any particular tickers. Each **event-agent** proposes its basket; the events are then ranked by conviction (against each other and versus the always-on SPY/gold floors detailed below), and since this solution manages at most `max_agents` (currently 7) concurrent event-agents, the lower-conviction agents and their tickers fade away. The surviving agents hand their tickers to the mechanical mean-variance **optimizer**, which sizes them into portfolio weights from each ticker's recent returns and volatility, zeroing out the weakest.
