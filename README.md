...
When **backtesting**, each weekly news-grab is fetched from **GDELT**, whose date bounds are server-enforced — so a scan as-of a past week sees only news published by then and can't leak future news into the backtest. (GDELT misses the niche early under-the-radar pieces, so those are **seeded** back at their true dates.) When this solution is run **forward in time** (week to week, for real), that look-ahead problem disappears — "search now for a just-happened gem" is clean by construction — so the firehose there is **Anthropic web search** instead. See [`agent_design.md`](agent_design.md#retrieval-gdelt-and-seeds-current) for the retrieval mechanics.
