...
- An **exit** is where the spine ends: the catalyst *resolves* (the war ends, the bill is signed, the chokepoint reopens; for the "Hormuz blockade," the ceasefire triggers the exit). The event-agent drops the position the instant it judges the catalyst done, even if the coverage is still loud, because a resolved catalyst can no longer drive the ticker.
